...
export const loginApi = (params: Login.ReqLoginForm) => {
	// 正常 post json 请求
	return http.post<Login.ResLogin>(PORT + `/user/login`, params, { noLoading: true });
};
...
export const logoutApi = () => {
	return http.post(PORT + `/user/logout`);
};

// * 用户注册
export const registerApi = (params: Login.ReqRegisterForm) => {
	return http.post<Login.ResLogin>(PORT + `/user/register`, params, { noLoading: true });
};
